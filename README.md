...
When a browser-enhanced form action mutates data, calls `revalidatePath()` for the current route, and returns normally, the navigation runtime asks the action endpoint for a single-flight mutation response. The server rerenders that same route as navigation HTML in the POST response, marks it with `x-mreact-action-single-flight`, and the browser applies the fresh route payload without issuing a second GET. Unsupported cases keep the existing `x-mreact-revalidate` invalidation and redirect/follow-up navigation fallback. This is separate from duplicate-submission coalescing or idempotency keys, which are application-level replay concerns.
...
The repository contains several benchmark tracks:

- [benchmarks/primitive](benchmarks/primitive): primitive UI/reactivity comparisons across Marko, Vue, Svelte, Angular, Qwik, React, Solid, and mreact.
- [benchmarks/primitive-browser](benchmarks/primitive-browser): real Chromium runs for DOM-heavy primitive cases so happy-dom-specific rankings can be cross-checked against browser behavior.
- [benchmarks/non-router](benchmarks/non-router): package-level microbenchmarks for store, virtual list, query, forms, auth, and other non-router packages.
- [benchmarks/router](benchmarks/router): app-router comparisons across Marko Run, Nuxt, SvelteKit, Analog, Qwik City, SolidStart, TanStack Start, Next.js App Router, and mreact app router.
- [benchmarks/lambda-route-latency](benchmarks/lambda-route-latency): local AWS Lambda adapter route latency reproduction for cold health checks, first redirects, and warm redirects.
...
| --- | --- |
| [examples/app-router](examples/app-router) | Full app-router tour: layouts, metadata, client boundaries (route-level `cell` inference and explicit `.client.tsx` boundaries when needed), streaming, server actions, cache, route handlers, middleware, auth, query, forms, i18n, deployment adapters |
