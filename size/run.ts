...
if (checkBudget) {
  ...
  if (failures.length > 0) {
    console.error(
      `\nPackage dist size budget failed:\n${failures.map((failure) => `- ${failure}`).join("\n")}`,
    );
    process.exitCode = 1;
  }
}
...
async function measureDirectory(
  directory: string,
): Promise<{ files: number; gzipBytes: number; rawBytes: number }> {
...
function printReport(
  rows: readonly PackageSizeRow[],
  totals: { totalGzipBytes: number; totalRawBytes: number },
): void {
  ...
  for (const row of rows) {
    console.log(
      `| ${row.name} | ${row.files} | ${formatBytes(row.rawBytes)} | ${formatBytes(row.gzipBytes)} |`,
    );
  }
  console.log(
    `| total | ${rows.reduce((sum, row) => sum + row.files, 0)} | ${formatBytes(totals.totalRawBytes)} | ${formatBytes(totals.totalGzipBytes)} |`,
  );
}
