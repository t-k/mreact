...
- `MREACT_ROUTER_TRUST_FORWARDED_PROTO=1` makes the Node server trust the first `X-Forwarded-Proto` value. Use it only behind a proxy that overwrites the header and blocks direct access to the Node port.
