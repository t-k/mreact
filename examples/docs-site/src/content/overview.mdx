...
It takes inspiration from [Marko](https://markojs.com/)'s HTML-first streaming model, [Solid](https://www.solidjs.com/)'s fine-grained reactivity, and [Qwik](https://qwik.dev/)'s focus on avoiding unnecessary browser work. It is not a resumability framework; instead, it uses compiler-assisted server/client splitting, fine-grained updates, and route-level inference to keep server-rendered routes free of browser runtime when they do not need it.
