...
Use `defer()` when a loader has critical data plus non-critical promises. Keep redirects, `notFound()`, and status-bearing `Response` results in the critical path before returning deferred data.

```tsx
import { defer, notFound, type LoaderContext } from "@reckona/mreact-router";
...
export default function UserPage(props: {
  readonly data: { readonly posts: Promise<Post[]>; readonly user: User };
}) {
  return (
    <main>
