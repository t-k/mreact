...
## Component slots during SSR

JSX passed through a named prop is rendered on the server with the same markup semantics as `children`. This is useful for component APIs with distinct regions such as `actions`, `filters`, or `sidebar`; Mreact preserves those regions through string rendering, streaming, and hydration instead of converting their HTML to text.

```tsx
function Detail(props: { actions: MReactNode; children: MReactNode }) {
  return (
    <section>
...
Keep values that cross a client boundary serializable. JSX regions can contain server-rendered markup, but ordinary data props still follow the boundary serialization contract.
