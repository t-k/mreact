...
After the route renders, Mreact can dehydrate the request query cache for client code that uses the same query key. Dehydrated data is embedded in the delivered HTML and is visible to the user. Pass `dehydrateOptions.shouldDehydrateQuery` to a programmatic render or adapter entry point to exclude server-only or sensitive queries. For Vite development, prerendering, generated Node output, and generated Lambda handlers, configure `dehydratePolicyModule` with a project-relative server module that exports a named `dehydrateOptions` object. Mreact bundles that executable policy into the server artifact instead of serializing a callback closure, and a missing or malformed configured module fails the build or server startup. Set a `staleTime` when the browser should reuse the server-fetched result during the initial mount instead of immediately revalidating it.

```ts title="src/dehydrate-policy.ts"
import type { DehydrateOptions } from "@reckona/mreact-query";

export const dehydrateOptions = {
  shouldDehydrateQuery: (entry) => entry.queryKey[0] !== "private",
