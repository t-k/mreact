...
Keep the loader return type explicit when the route data becomes part of the page contract. This makes `props.data` readable and keeps metadata, client boundaries, and tests aligned with the same shape.
...
Use `notFound()` when a loader determines that the current route should render the nearest `not-found.tsx` boundary. Use a `Response` when the request should become a redirect or another custom HTTP response.
...
The loader receives a per-request query client. Use it when server data should also hydrate client-side query observers.
