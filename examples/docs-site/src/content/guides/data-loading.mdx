...
For browser-only coordination after hydration, `@reckona/mreact-query` also provides `syncQueryClientAcrossTabs()`. That adapter works across same-origin browser tabs and is separate from the per-request loader client: use it when focus or reconnect refetches should single-flight across tabs, and scope its channel to the current user or tenant before sharing query data.
