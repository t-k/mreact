...
## Customize every completed response

Create `src/app/on-response.ts` when an app needs to inspect or replace completed responses consistently in `mreact-router dev`, `mreact-router start`, and built request rendering. Export an `onResponse(response, { request })` function and return a replacement `Response`, or mutate the provided response and return nothing.

```ts
// src/app/on-response.ts
export function onResponse(response: Response, { request }: { request: Request }) {
...
Use middleware for decisions that happen before rendering, such as redirects and rewrites. Use `on-response.ts` for policy that must see the final page, middleware, or route response. A programmatic `onResponse` option passed to a renderer or adapter takes precedence over the convention, so a custom worker or handler can import and reuse the same function when it needs platform-specific setup.
