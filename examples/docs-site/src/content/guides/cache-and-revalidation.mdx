...
For server actions today, revalidation can invalidate the cached route HTML and the client navigation cache. Mreact does not yet attach refreshed route payloads to the server action response, so unsupported flows can still need a follow-up navigation or data request after the mutation response.
