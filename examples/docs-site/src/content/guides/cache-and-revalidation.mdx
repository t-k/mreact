...
When a route, a layout, or one of their local server imports reads request headers or constructs `new Request(request)`, Mreact conservatively opts that route out of shared HTML caching. Reads through opaque external packages or ambient process state cannot always be proven from the local source closure. Keep those routes uncached with `export const revalidate = 0`, or move the request-dependent value outside shared rendered HTML.
