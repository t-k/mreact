...
- `MREACT_ROUTER_TRUST_FORWARDED_PROTO=1`: trust the first `X-Forwarded-Proto` value on Node only when a trusted proxy overwrites it and the Node port is not publicly reachable.
