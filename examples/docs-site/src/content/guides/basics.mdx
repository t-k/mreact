...
For a new Mreact application, begin with the compiled App Router model and native primitives: use `cell()` for writable state, `computed()` for derived values, and `effect()` for imperative side effects. The package names describe authoring APIs, while the compiler decides which parts execute on the server and which parts hydrate in the browser.

| Need | Starting point | Execution and ownership |
| --- | --- | --- |
| New Mreact UI | App Router with `cell`, `computed`, and `effect` | Native compiled boundaries; keep state inside the component or cleanup owner that owns it. |
| React-style code or an ecosystem library | `@reckona/mreact-compat` or `@reckona/mreact` compatibility exports | Compatibility execution is appropriate for migration and verified library scope; test the behavior you depend on. |
| Direct DOM or imperative integration | `@reckona/mreact-reactive-dom` with `createRoot()` and explicit disposal | The root or `createCleanupScope()` owns bindings, listeners, subscriptions, and other resources. |

The native and compatibility paths can implement the same counter behavior. Native authoring uses tracked cell reads:
...
React-style authoring uses the compatibility hook contract:
...
Both examples describe a counter; optimization and fallback decisions may change the shipped work, but they preserve the observable state and event behavior covered by the selected API contract.
...
Native components mounted through JSX run their setup without subscribing the surrounding render scope to incidental reads. A synchronous helper called from a component body can read a cell without causing the parent conditional to recreate that component when the cell changes. JSX bindings and explicit effects still track their own reads, and changing a parent condition can unmount and mount the component again. This isolation applies to JSX component calls; an explicit JavaScript call such as `effect(() => App())` retains ordinary effect tracking.

Use the existing `untrack()` helper when an imperative operation needs a snapshot inside an otherwise tracked scope:

```ts
import { cell, effect, untrack } from "@reckona/mreact-reactive-core";

const query = cell("");
const setupInFlight = cell(false);

const dispose = effect(() => {
  const currentQuery = query.get();
  const busy = untrack(() => setupInFlight.get());
  console.log(currentQuery, busy);
});

// Call dispose() when this manually owned effect is no longer needed.
```

Here, changing `query` reruns the effect; changing `setupInFlight` alone does not.
...
### Resources that outlive a component evaluation

`computed()`, effects, query observers, and the computeds inside virtual grids register with the cleanup owner active when they are constructed. A component evaluation can have a shorter lifetime than the component or a controller retained by your application: reevaluation can dispose that evaluation's resources. Keeping an object in a variable does not change its cleanup owner.

Use `runDetached()` to construct a resource with an explicitly managed lifetime, including lazy construction from a component body:

```ts
import { effect, runDetached } from "@reckona/mreact-reactive-core";

const dispose = runDetached(() => effect(() => console.log(count.get())));
// The application owner must call this when the resource is no longer needed.
dispose();
```

For a controller that owns a query and a virtual grid, create an independent `createCleanupScope()`, build both inside `runWithCleanupScope(controllerScope, ...)`, and expose `controllerScope.dispose()` as the controller's disposal method. This also disposes the virtual grid's internal computeds, which have no separate virtualizer disposal method. Creating a cleanup scope does not attach it to the surrounding component evaluation.

`runDetached()` only removes the implicit cleanup owner during the synchronous callback. It restores the previous owner even if construction throws, preserves dependency tracking, and allows an explicit nested cleanup scope. It does not extend ownership across `await`, detach DOM nodes or bindings, or make state safe to share between SSR requests. Keep user-specific controllers inside their request or client lifetime; never retain them in server module globals.
