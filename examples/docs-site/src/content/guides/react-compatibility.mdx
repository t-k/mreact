...
The repository also includes focused compatibility labs for ecosystem packages that need screenshot and DOM-summary comparison against React. Run `pnpm compat-lab:radix` when changing Radix-facing event or portal behavior, and run `pnpm compat-lab:recharts` when changing chart, SVG, animation, or tooltip behavior.
