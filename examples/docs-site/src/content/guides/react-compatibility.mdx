...
Treat `optimizeDeps.include` as a dev-experience tool, not a compatibility switch. If a library imports browser globals during server render, move that usage behind a client boundary or isolate the browser-only code in a `.compat.tsx` component.
...
Use this as a performance model, not a compatibility switch: start from server-renderable route code, isolate browser-only libraries behind client boundaries, and check the route's client bundle size on the [Benchmarks](/benchmarks/) page or in your own production build.
