...
Since `0.0.151`, concrete `Link` hrefs can be typed directly. Build output includes `.mreact/routes.d.ts`, which declares discovered app route patterns and augments `@reckona/mreact-router/link`. Include that declaration in your TypeScript program, and `Link` will type-check internal href strings without importing any generated runtime module.
