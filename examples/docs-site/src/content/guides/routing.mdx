...
## Page and layout location

Page and layout props expose `request` as a serializable `RouteLocation` with `url`, `pathname`, `search`, and `hash`. These fields describe the current route in both server rendering and browser hydration. Cached HTML carries only a pathname placeholder; hydration and client navigation restore the complete browser URL before shared code runs.

```tsx
import type { PageProps, RouteLocation } from "@reckona/mreact-router";

type Data = { readonly title: string };

export default function Page(props: PageProps<Data>) {
  const location: RouteLocation = props.request;

  return <main>{location.pathname}{location.search}</main>;
}
```

Use `LoaderContext`, `GenerateMetadataContext`, or a route handler when code needs the server-only `Request` object, including headers, cookies, body, credentials, or an abort signal.
