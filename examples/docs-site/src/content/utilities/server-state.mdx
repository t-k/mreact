...
Use `queryDefinition()` when one query key and data type should be shared across loaders, prefetches, cache reads, and updates. The definition-bound APIs infer the cached data type; raw-key APIs remain available for dynamic caches.
...
const profile = queryClient.getQueryData(profileQuery);
queryClient.setQueryData(profileQuery, (previous) => ({
  ...(previous ?? { id: userId, name: "" }),
  name: "Ada Lovelace",
}));
```
...
`refetchOnWindowFocus` listens for focus and visible-tab transitions. `refetchOnReconnect` listens for the browser `online` event. `refetchOnInvalidate` lets an active observer refetch when its query key is invalidated, which is useful when a mutation updates server state and the visible view should refresh without waiting for focus, reconnect, or a manual `refetch()`. `refetchInterval` schedules the next poll after the preceding fetch settles, so requests never overlap; it pauses while the document is hidden and resumes without replaying missed ticks. Set `refetchIntervalInBackground: true` only when hidden-tab polling is required. Always dispose long-lived observers when integrating outside a component cleanup lifecycle.
...
For a client-wide memory policy, pass `inactiveGcTime` to `createQueryClient()`. It expires unused successful or failed prefetch entries after they settle. `maxInactiveEntries` adds a count cap that evicts the oldest settled inactive entries while preserving active observers. Both settings are opt-in and default to the existing unlimited retention behavior.
