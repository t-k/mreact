...
`exportStaticApp()` writes one `index.html` per prerendered route and copies generated client assets into the export directory. Only prerendered routes are exported. Before replacing the existing export directory, Mreact validates every selected path, the current prerender schema, shared-cache-safe response headers, and a successful status code; an invalid or legacy entry fails the export and leaves the previous directory intact. Dynamic runtime APIs, route handlers, request-time auth, cookies, and middleware decisions need a server, Worker, or Lambda target.
