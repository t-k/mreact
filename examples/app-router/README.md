...
The generated Cloudflare asset loader intentionally forwards only files listed in the generated client manifest (`manifest.json`, hashed route scripts, route CSS, copied public assets, and linked source maps when `clientSourceMaps: "linked"` is enabled). Requests such as `/_mreact/client/../secret.js` or encoded traversal variants are rejected before reaching the `ASSETS` binding.
