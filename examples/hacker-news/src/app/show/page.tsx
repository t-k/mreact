...
export async function loader(): Promise<FeedData> {
  const result = await hn.getStories("show", 30);
  if (result.isErr()) return { kind: "error", message: "Could not load Show HN." };
...
export default function Page(props: { data: FeedData }) {
  return (
    <main>
      <h1 class="mb-3 text-xl font-semibold text-stone-950">Show HN</h1>
      {props.data.kind === "error" ? (
        ...
        <p role="alert" class="text-sm text-stone-600">
          No Show HN stories are visible.
        </p>
      ) : (
