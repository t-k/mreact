...
export async function loader(): Promise<FeedData> {
  const result = await hn.getStories("ask", 30);
  if (result.isErr()) return { kind: "error", message: "Could not load Ask HN." };
...
export default function Page(props: { data: FeedData }) {
  return (
    <main>
      <h1 class="mb-3 text-xl font-semibold text-stone-950">Ask HN</h1>
