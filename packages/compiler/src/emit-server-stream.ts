    ...
    .map((component) => {
      const emitted = emitComponent(
        ...
        compatRenderToStringHelperName,
        {
          ...
          ...(selectionMultipleParameterName === undefined
            ? {}
...
function emitComponent(
  ...
  const appendStatements = emitAppendStatements(
    ...
    options.selectionMultipleParameterName,
  );
...
function emitAppendStatements(
  ...
  selectionMultipleParameterName: string | undefined,
): string[] {
  if (node.kind === "conditional") {
  ...
  const collectState: CollectHtmlState = {
    ...
    ...(selectionMultipleParameterName === undefined
      ? {}
      : { selectedMultipleCode: selectionMultipleParameterName }),
  };
...
function collectHtmlParts(
  ...
  const childSelectedValueCode =
    node.tagName === "select"
      ...
      : undefined;
  const childSelectedMultipleCode =
    node.tagName === "select"
      ? node.attributes.some((attr) => attr.kind === "spread-attr")
...
function emitMergedSpreadElementPart(
  ...
  const omitSelectedCode =
    tagName === "option" && selectedValueCode !== undefined
  ...
  const opening = `${stringLiteral(`<${tagName}`)} + ${currentSpreadAttributesHelperName}(${stringLiteral(tagName)}, ${propsName}, ${omitSelectedCode})${selectedExpression === undefined ? "" : ` + (${selectedExpression})`} + ">"`;
...
function emitCapturedOptionPart(
  node: Extract<JsxNodeIr, { kind: "element" }>,
  escapeHelperName: string,
  state: CollectHtmlState,
  attributeScan: ElementAttributeScan,
  ...
  const textValueName = capturedOptionText.name;
  const attributes = collectElementAttributeParts(
    node.tagName,
    node.attributes,
    escapeHelperName,
  ...
  if (attributeExpressions.some((expression) => expression === undefined)) {
    return undefined;
  }
  const attributesCode =
    attributeExpressions.length === 0 ? '""' : (attributeExpressions as string[]).join(" + ");
  ...
  const selectedAttribute = emitOptionSelectedAttributeCode(
    state.selectedValueCode,
    optionValueCode,
...
function findCapturableOptionTextChildren(
  node: Extract<JsxNodeIr, { kind: "element" }>,
): CapturableOptionTextChild[] | undefined {
  ...
  for (const child of node.children) {
    ...
    if (
      ...
      !isChildrenExpressionCode(child.code)
    ) {
      hasDynamicText = true;
      continue;
    }
  ...
  return hasDynamicText ? (node.children as CapturableOptionTextChild[]) : undefined;
}
  ...
  const parts = children.flatMap((child) =>
    collectHtmlParts(
      ...
      "_renderReactSuspenseOutOfOrderBoundary",
      {
        ...
        ...(selectedMultipleCode === undefined ? {} : { selectedMultipleCode }),
      },
    ),
  );
  const expressions = parts.map((part) =>
...
function emitStreamRendererFromChildren(
  ...
  const childState: CollectHtmlState = {
    ...
    ...(selectedValueCode === undefined
      ...
      : { selectedValueCode }),
    ...(selectedMultipleCode === undefined
      ? parentState?.selectedMultipleCode === undefined
        ? {}
        : { selectedMultipleCode: parentState.selectedMultipleCode }
      : { selectedMultipleCode }),
  };
  const parts = children.flatMap((child) =>
    collectHtmlParts(
