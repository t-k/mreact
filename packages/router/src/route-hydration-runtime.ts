/**
 * Feature-scoped virtual modules that carry the route-independent half of the generated client route entry.
 *
 * `buildClientRouteEntrySource` used to inline every hydration helper into each route entry, so a five route session downloaded five byte-identical copies of the same resume walk. The helpers now live here and every bundler path resolves them through the same specifier, which lets a multi-route build hoist them into a single shared chunk while a single-route build still inlines them.
 *
 * The split is per capability rather than one always-loaded runtime: a route that never streams out-of-order fragments and never renders a client boundary imports neither of those modules, and the route-scoped event/dom-ref synchronisers stay generated in the entry and are injected into the resume factory so a route without dom refs never pulls `getDomRefBindings`.
 */

/** Runtime module names, one per independently importable capability group. */
...
/** Import specifier a generated route entry uses to reach one runtime module. */
...
/** Resolves a specifier or a bundler path back to a runtime module name. */
...
/** Source of one runtime module. The helper bodies are the generated originals verbatim. */
export function routeHydrationRuntimeSource(module: RouteHydrationRuntimeModule): string {
  return routeHydrationRuntimeSources[module];
}

const routeHydrationRuntimeSources: Record<RouteHydrationRuntimeModule, string> = {
  boundaries: `/**
...
`,
  fragments: `export function __mreactApplyOutOfOrderFragments(root) {
...
`,
  lifecycle: `export function __mreactRunLifecycleTasks(values, run) {
  let firstError;
...
`,
  resume: `import { __mreactRunLifecycleTasks } from "mreact-route-hydration-runtime/lifecycle";
