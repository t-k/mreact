...
export default function Page() {
  ...
  return (
    <main>
      ...
      <input id="pristine-static" defaultValue="static-seed" />
...
test.describe.serial("pre-hydration input preservation", () => {
  ...
  test("an uncontrolled defaultValue keeps pre-hydration keystrokes through hydration", async ({
    ...
    // An untouched defaultValue still shows its server value afterwards, whether
    // the seed was bound or static. The static one used to be wiped, because the
    // client template carried the raw JSX prop name and hydration replaced the
    // server's `value` with a parsed-lowercase `defaultvalue`.
    expect(await inputValue(page, "pristine-bound")).toBe("server-seed");
    expect(await inputValue(page, "pristine-static")).toBe("static-seed");
  });
