...
describe("bindStaticKeyedSingleNodeList", () => {
  ...
  test("keeps static compiler row masks current for text, events, and escaped contexts", async () => {
    type Item = { readonly id: number; readonly label: string };
    const items = cell<readonly Item[]>([
      { id: 1, label: "A" },
      { id: 2, label: "B" },
    ...
    const contexts = new Map<
      number,
      { readonly index: number; readonly item: Item; readonly items: readonly Item[] }
    >();
    parent.append(marker);
    ...
    const dispose = bindCompilerKeyedSingleNodeList(
      ...
      () => items.get(),
      (context) => {
        contexts.set(context.item.id, context);
        const row = document.createElement("tr");
        const text = document.createTextNode("");
        const button = document.createElement("button");
        bindText(text, () => `${context.item.label}:${context.index}:${context.items.length}`);
      ...
      { key: (item) => item.id, compilerRowReadMask: 7 },
    );
    await flushEffects();

    const firstRow = parent.children[0];
    const firstContext = contexts.get(1);
    items.set([
      { id: 2, label: "B!" },
      { id: 1, label: "A!" },
    ]);
    ...
    expect(parent.children[1]).toBe(firstRow);
    expect(firstRow?.textContent).toBe("A!:1:2");
    expect(firstContext?.item.label).toBe("A!");
    expect(firstContext?.index).toBe(1);
    expect(firstContext?.items).toBe(items.get());
    firstRow?.querySelector("button")?.click();
    expect(payloads).toEqual(["A!:1:2"]);
  ...
  test("tracks external dependencies through a static compiler row mask", async () => {
    const label = cell("A");
    const items = cell<readonly { readonly id: number; readonly label: string }[]>([
      {
        id: 1,
    ...
    const dispose = bindCompilerKeyedSingleNodeList(
      ...
      (context) => {
        const row = document.createElement("tr");
        const text = document.createTextNode("");
        bindCompilerKeyedText(context, text, () => context.item.label);
        row.append(text);
        return row;
      },
  ...
  test("notifies a late effect through an escaped static compiler context", async () => {
    type Item = { readonly id: number; readonly label: string };
    const first = { id: 1, label: "A" };
    const items = cell<readonly Item[]>([first]);
    const parent = document.createElement("tbody");
    const marker = document.createComment("rows");
    let escapedContext:
      | { readonly index: number; readonly item: Item; readonly items: readonly Item[] }
    ...
    const disposeList = bindCompilerKeyedSingleNodeList(
      parent,
      marker,
      () => items.get(),
      (context) => {
    ...
    await flushEffects();
    const targetContext = escapedContext;
    const values: string[] = [];
    const disposeEffect = effect(() => {
      const context = targetContext;
      if (context !== undefined) {
        values.push(`${context.item.label}:${context.index}:${context.items.length}`);
      }
    });
    expect(values).toEqual(["B:0:1"]);
    ...
    await flushEffects();
    expect(values).toEqual(["B:0:1", "B!:1:2"]);

    disposeEffect();
    disposeList();
  });

  test("learns late row reads on the dynamic compiler context fallback", async () => {
    type Item = { readonly id: number; readonly label: string };
  ...
  test("keeps static and dynamic compiler contexts on the same own Symbol shape", () => {
    const createContextKeys = (compilerRowReadMask: number | undefined): PropertyKey[] => {
      const parent = document.createElement("tbody");
      const marker = document.createComment("rows");
      let contextKeys: PropertyKey[] = [];
      parent.append(marker);
      const dispose = bindCompilerKeyedSingleNodeList(
        parent,
        marker,
        ...
        {
          key: (item) => item.id,
          ...(compilerRowReadMask === undefined ? {} : { compilerRowReadMask }),
        },
      );
      dispose();
      return contextKeys;
    ...
    const dynamicKeys = createContextKeys(undefined);
    const staticKeys = createContextKeys(0);
    expect(staticKeys).toEqual(dynamicKeys);
    expect(staticKeys).toHaveLength(4);
    expect(staticKeys.every((key) => typeof key === "symbol")).toBe(true);
  });
  ...
  test("stores compiler row state only in its compiler context", async () => {
    ...
    expect(source).not.toContain("compilerRowHasStaticReads");
    expect(source).toContain("Object.create(rowContextPrototype)");
