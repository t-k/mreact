...
describe("createAppRouterImportPolicyPlugin", () => {
  ...
  test("@modular-react/app-router is rewritten to the in-repo index.ts", () => {
    const resolve = makePlugin(appDir, { appDir, label: "server" });
    const result = resolve("@modular-react/app-router");
    expect(result?.path).toMatch(/app-router\/src\/index\.ts$/);
  });
