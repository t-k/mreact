...
The adapter targets API Gateway HTTP API v2 and Lambda Function URL payload format 2.0.
...
`dist/lambda.mjs` exports `handler`. Package that file together with `.mreact`, `package.json`, and production `node_modules`.
...
The Lambda runtime must provide Web `Request` and `Response`. Use a current Node.js Lambda runtime.
...
Lambda can serve `.mreact/client`, but production deployments should usually move static assets to S3 + CloudFront or another CDN.
...
Keep the Lambda bundle focused on the handler and server runtime. The `.mreact` directory is build output and must be deployed with the handler because it contains the server manifest, route modules, and client asset manifest.
