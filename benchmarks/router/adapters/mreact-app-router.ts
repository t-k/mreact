...
export default function Page() {
  ...
  server = await startServer({ outDir, port: 0, sinkStrategy });
  currentNodeCount = nodeCount;
  return server.url;
}

export const mreactAppRouterAdapter: AppFrameworkAdapter = {
  ...
  async renderWaterfall(): Promise<string> {
    const url = await ensureFixture(NODE_COUNT_DEFAULT);
