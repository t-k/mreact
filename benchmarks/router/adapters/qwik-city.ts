// qwik-city adapter (Qwik 1.19 + Qwik City + Vite 7 + node-server adapter).
//
// Qwik City は Vite 5-7 互換 (Vite 8 非対応)。fixture を tmp に作って
...
async function ensureFixture(nodeCount: number): Promise<string> {
  ...
  rootDir = await mkdtemp(join(fixtureParent, "qwik-city-fixture-"));
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        name: "mreact-bench-qwik-city-fixture",
        private: true,
        type: "module",
        scripts: {
...
`,
  ...
  if (!ready) {
    child.kill();
    throw new Error(`qwik-city server did not become ready in 30s: ${stderr.slice(-2000)}`);
  }
...
export const useTargetData = routeLoader$(() => ({ label: "loaded-target" }));
export default component$(() => {
  const data = useTargetData();
  return <main><h1>Navigation target</h1><p>loader:{data.value.label}</p></main>;
});
`,
  );
...
function assertLastSpan(html: string, nodeCount: number, label: string) {
  ...
  if (!html.includes(`>${nodeCount - 1}</span>`)) {
    throw new Error(`qwik-city ${label} did not include the last node`);
  }
}

export const qwikCityAdapter: AppFrameworkAdapter = {
  name: "qwik-city",
  ...
  async renderWaterfall(): Promise<string> {
    ...
    if (!html.includes(`data-a="A"`) || !html.includes(`data-b="B"`)) {
      throw new Error("qwik-city renderWaterfall did not include both branches");
    }
    return html;
  },
  async renderDynamicAttrGrid(cellCount: number): Promise<string> {
    ...
    if (!html.includes(`Item #${cellCount - 1} &lt;data`)) {
      throw new Error("qwik-city renderDynamicAttrGrid did not include the last escaped text");
    }
    return html;
  },
  ...
  async measureBuildOutputGzipBytes(): Promise<number> {
    ...
    if (rootDir === undefined) {
      throw new Error("qwik-city fixture not initialized");
    }
